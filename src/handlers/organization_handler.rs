use crate::models::{
    AllStaff, BasicResponse, Count, Filter, Oiv, OivRequest, OrgResponse, Organization,
...
pub async fn get_organizations(
...
) -> (Status, Json<BasicResponse<Vec<OrgResponse>>>) {
    ...
    let mut res: Vec<OrgResponse> = Vec::new();

    for oiv in oivs {
        let mut arr: Vec<Organization> = Vec::new();
        if oiv.id == 3 {
            arr.push(orgs[0].clone());
        }
        arr.push(orgs[(oiv.id - 1) as usize].clone());
    ...
    (Status::Ok, Json(BasicResponse::ok(res)))
}
